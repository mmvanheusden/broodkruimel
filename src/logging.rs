...
pub fn info(message: impl Into<String>, tag: Option<impl Into<String>>) {
    match tag {
        Some(tag) => println!("ℹ️ {}{}: {}", "INFO".bold(), format!("({})", tag.into().blue().bold()), message.into().bright_white().bold()),
...
pub fn error(message: impl Into<String>, tag: Option<impl Into<String>>) {
    match tag {
        Some(tag) => println!("❌ {}{}: {}", "ERROR".red().bold(), format!("({})", tag.into().blue().bold()), message.into().bright_red().bold()),
